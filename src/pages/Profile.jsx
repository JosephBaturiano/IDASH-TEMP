...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    axios.request(config)
      .then(async (response) => {
        ...
        // Fetch badgeOne and badgeTwo images using their IDs
  ...
  // Helper function to fetch image URL by ID
  const fetchImageUrl = async (imageId) => {
    try {
      const response = await axios.get(`http://edl-wp1.local/wp-json/wp/v2/media/${imageId}`);
      return response.data.source_url; // Get the image URL from the response
    } catch (error) {
        ...
        <div className="bg-[#dbedff] rounded-lg shadow-md h-[180px] w-[600px] overflow-hidden"> {/* Ensure the container hides overflow */}
          ...
          <div className="flex gap-4 px-4 pt-1">
            {profileData.badgeOne && (
              <img src={profileData.badgeOne} alt="Badge 1" className="h-[130px] w-[130px]" />
            )}
            {profileData.badgeTwo && (
              <img src={profileData.badgeTwo} alt="Badge 2" className="h-[130px] w-[130px]" />
            )}
            {profileData.badgeThree && (
              <img src={profileData.badgeThree} alt="Badge 3" className="h-[130px] w-[130px]" />
            )}
          </div>
        </div>
        ...
        <div className="bg-[#dbedff] rounded-lg shadow-md h-[150px] w-[600px]">
          ...
          <div className="flex gap-8 px-4 pt-1">
            <a href="https://github.com/vt4b" target="_blank" rel="noopener noreferrer">
              <img src="src/assets/github-logo.png" alt="github" className="h-[60px] w-[60px]" /> {/* Updated URL */}
            </a>
            <a href="https://discord.com" target="_blank" rel="noopener noreferrer">
              <img src="src/assets/discord-logo.png" alt="discord" className="h-[60px] w-[60px]" /> {/* Updated URL */}
            </a>
            <a href="https://drive.google.com/drive/folders/1wKjQNXLYTmaqTpsEOyZgol4JpFmnvjfW" target="_blank" rel="noopener noreferrer">
              <img src="src/assets/gdrive-logo.png" alt="gdrive" className="h-[60px] w-[60px]" /> {/* Updated URL */}
            </a>
          </div>
        </div>
      </div>
