...
const TimeSheetCard = ({ item, onEdit, onToggleInclude }) => {
    ...
    return (
        <tr key={item.id} className={`${bgColor} ${borderColor} ${hoverColor} border-t transition-colors duration-300`}>
            ...
            <td className={`px-2 py-2 ${textColor} text-center break-words max-w-[150px]`}> {/* Updated here */}
                {item.description}
            </td>
            ...
            <td className={`px-2 py-2 ${textColor} text-center break-words max-w-[158px]`}> {/* Updated here */}
                {item.deliverables}
            </td>
