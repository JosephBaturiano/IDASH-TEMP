        ...
        <tbody>
          {tasks.map((task) => (
            <tr key={task.id}>
              ...
              <td className="border px-4 py-2 text-center">{task.acf ? task.acf.assigned_to : 'Not assigned'}</td>
              <td className="border px-4 py-2 text-center">
                <select
                  value={task.acf ? task.acf.status : 'No status'}
                  onChange={(e) => handleStatusChange(e, task.id)}
                  className="bg-white border border-gray-300 rounded-full px-2 py-1 text-center"
                >
